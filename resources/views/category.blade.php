    ...
    <div class="flex flex-wrap justify-between">
        ...
        <div class="flex flex-col mt-1.5 lg:text-sm lg:w-56 text-xs w-[48%] border items-center justify-between text-center space-y-2 p-2">
            ...
            <div class="flex w-4/5  mb-2 text-xs font-semibold ">
                <button @if(Auth::check() == false) @else onclick="addToCart({{$product->id}},'{{route('add_to_cart',$product->id)}}','{{url('images/products/'.$product->image)}}','{{route('get_item',$product->id)}}')" @endif  class="flex bg-white border drop-shadow-sm w-full text-gray-900 items-center justify-center space-x-1 p-2 rounded">
